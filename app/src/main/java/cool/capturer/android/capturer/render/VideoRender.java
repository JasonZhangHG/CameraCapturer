...
import cool.capturer.android.capturer.capture.VideoCaptureFrame;
import cool.capturer.android.capturer.connector.SinkConnector;
import cool.capturer.android.capturer.connector.SrcConnector;
import cool.capturer.android.capturer.gles.ProgramTexture2d;
import cool.capturer.android.capturer.gles.ProgramTextureOES;
import cool.capturer.android.capturer.gles.core.GlUtil;
import cool.capturer.android.capturer.util.FPSUtil;
...
public class VideoRender implements SinkConnector<VideoCaptureFrame> {
    ...
    //这个矩阵可以镜像纹理坐标
    //This Matrix will apply horizontal flip to texture coordinate.
        ...
        public void onDrawFrame(GL10 gl) {
            if (mNeedsDraw) {
                ...
                try {
                    ...
                    if (frame.mMirror) {
                        //OpenGL matrix 的memory layout 和cpu 上的有transpose 关系，所以本来是Mirror * Rotation 但是在这里计算要调换顺序。
                        Matrix.multiplyMM(mMTX, 0, mMTX, 0, mirrorMatrix, 0);
                    }
